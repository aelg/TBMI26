...
%   Please see the solution for exercise 5.1 (class 5) for
%   the description of the network variables.
function [train_out, test_out, eval_out] = backprop_lab(X, D, X_test, D_test, X_eval)
...
for iter=1:Nep
   ...
   grad_v = zeros(size(V)); %Calculate the gradient for the output layer
   grad_w = zeros(size(W)); %..and for the hidden layer.
   W = W; %Take the learning step.
   V = V; %Take the learning step.
