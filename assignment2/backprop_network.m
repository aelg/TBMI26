...
function [U, Y, S] = evalNet(X,V,W)
   % Calculate S the output from the hidden layer before the sigmoid
   % function
   S = W*X;
   % Push S through the sigmoid function.
   Y = tanh(S);
   % Add the "Y0" term that takes care of the constant term. Probably
   % wrong.
   % Y(1,:) = ones(1,size(X,2));
   % Push through the output layer.
   U = V*Y;
end
...
V = 0.1*randn(dim_out,dim_hidd + 1);
W = 0.1*randn(dim_hidd + 1,dim_in + 1);
...
%Main training loop
for iter=1:Nep
   [U, Y, S] = evalNet(X,V,W); %Evaluate the current network on the training data
   E(iter) = 1.0/(N*dim_out)*sum(sum((U-D).^2)); %Calculate the training error
   train_out = U;
   
   %Implement this! (Remove the stand in code...)
   %-----------------------------------------------
   %Remember the exercises! (5.1)
   grad_v = (1/N)*2*(V*Y-D)*Y'; %Calculate the gradient for the output layer
   grad_w = 2/N*((V'*(U-D)).*(ones(size(Y))-Y.^2))*X'; %..and for the hidden layer.
   W = W - eta*grad_w; %Take the learning step.
   V = V - eta*grad_v; %Take the learning step.
